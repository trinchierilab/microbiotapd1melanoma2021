# ML all_vs_all on Summarized Experiment
...
# summarized experiment object of data to analyze
...
for(study in sort(unique(lkt$Study))) {
  for(method in c("rf","svmPoly","glm")) {
    ...
    for(target in sort(unique(lkt$Study))) {
      if (target==study) {
        line <- append(c(Train=study, Test=target, Method=method), mdl$stats)
      }
      else {
        line <- append(c(Train=study, Test=target, Method=method),
                       testModel(mdl$model, 
                                 lkt[,lkt$Study==target]))
      }
      if (is.null(results)) {
